...
from java_generator_common import (
    PACKAGED_RESOURCES,
...
PACKAGED_OUTPUT_DIR = PACKAGED_RESOURCES / "examples" / "appendix_d"
SOURCE_URLS = (
    "https://www.fixtrading.org/online-specification/order-state-changes/",
    "https://fixtrading.org/online-specification/order-state-changes/",
    "https://www.fixtrading.org/online-specification/trade-appendix/",
    "https://fixtrading.org/online-specification/trade-appendix/",
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--source-html",
        default=os.environ.get("FIXDECODER_APPENDIX_D_HTML"),
        help=(
            "read a saved FIX Appendix D HTML page instead of fetching the "
            "current FIX Trading site"
        ),
    )
    args = parser.parse_args()

    html_text = load_source_html(args.source_html)
    if html_text is None:
        expected_files = load_generated_files(OUTPUT_DIR)
        if not expected_files:
            print(
                "Appendix D source is unavailable and no checked-in corpus exists",
                file=sys.stderr,
            )
            return 1
        print(
            "Appendix D source is unavailable; preserving the checked-in corpus",
            file=sys.stderr,
        )
    else:
    ...
    if args.check:
        packaged_expected_files = relocate_files(
            expected_files,
            OUTPUT_DIR,
            PACKAGED_OUTPUT_DIR,
        )
        if (
            load_generated_files(OUTPUT_DIR) != expected_files
            or load_generated_files(PACKAGED_OUTPUT_DIR) != packaged_expected_files
        ):
    ...
    print(
        "Wrote "
        f"{len(expected_files)} generated Appendix D sample files under "
        f"{OUTPUT_DIR} and {PACKAGED_OUTPUT_DIR}"
    )
    return 0


def load_source_html(source_html: str | None) -> str | None:
    if source_html:
        return Path(source_html).read_text()
...
def fetch_text(url: str) -> str | None:
    ...
    try:
        ...
        return fetch_text_with_curl(url, urllib_error)


def fetch_text_with_curl(url: str, urllib_error: urllib.error.URLError) -> str | None:
    try:
        result = subprocess.run(
            [
                ...
                "--show-error",
                "--user-agent",
                "fixdecoder Appendix D sample generator",
                "--max-time",
                "30",
                url,
            ],
        ...
        print(f"Unable to fetch {url}: {urllib_error}", file=sys.stderr)
        return None
    ...
    print(
        f"Unable to fetch {url}: {urllib_error}; curl: {result.stderr.strip()}",
        file=sys.stderr,
    )
    return None
...
def write_outputs(expected_files: dict[Path, str]) -> None:
    write_generated_files(
        expected_files,
        OUTPUT_DIR,
        ensure_dirs=[OUTPUT_DIR / "general", OUTPUT_DIR / "exchange"],
    )
    write_generated_files(
        relocate_files(expected_files, OUTPUT_DIR, PACKAGED_OUTPUT_DIR),
        PACKAGED_OUTPUT_DIR,
        ensure_dirs=[PACKAGED_OUTPUT_DIR / "general", PACKAGED_OUTPUT_DIR / "exchange"],
    )
