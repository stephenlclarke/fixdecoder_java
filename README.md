...
```text
Usage: fixdecoder [--xml=<xmlFiles>]... [--fix=<fixVersion>] [--info]
...
Pretty-print FIX log messages and inspect FIX dictionaries.
      ...
      [<files>...]          FIX log files, or stdin when omitted.
      --xml=<xmlFiles>      Load a custom FIX XML dictionary.
      --fix=<fixVersion>    Default FIX dictionary version.
      --info                Show loaded dictionary summary.
      --message[=<message>] Show message definition by name or MsgType.
      --component[=<component>]
                            Show component definition by name.
      --tag[=<tag>]         Show tag definition by number.
      --column              Compatibility flag for column output.
      --verbose             Show enum details in dictionary modes.
      --header              Include message header in message display.
      --trailer             Include message trailer in message display.
      --colour[=<colour>]   Colour mode: yes, no, always, never, or auto.
      --delimiter=<delimiter>
                            Input/output delimiter.
      --style=<style>       Compatibility flag for bat-style decorations.
      --plain               Compatibility flag for plain output.
      --number              Compatibility flag for line numbers.
      --paging=<paging>     Compatibility flag for pager mode.
      --pager=<pager>       Compatibility flag for pager command.
      --nowrap              Compatibility flag for pager wrapping.
      --follow              Follow files as they grow.
      --validate            Validate messages while decoding.
      --secret              Obfuscate sensitive values in decoded output.
      --secret-files        Write obfuscated .secret copies of input files.
      --summary             Show order summaries while decoding.
      --nocounts            Suppress final message count summary.
      --secret-dir=<secretDir>
                            Directory for --secret-files output.
...
- Supported long options: `--colour`, `--column`, `--component`, `--delimiter`, `--fix`, `--follow`, `--header`, `--help`, `--info`, `--message`, `--nocounts`, `--nowrap`, `--number`, `--pager`, `--paging`, `--plain`, `--secret`, `--secret-dir`, `--secret-files`, `--style`, `--summary`, `--tag`, `--trailer`, `--validate`, `--verbose`, `--version`, `--xml`
